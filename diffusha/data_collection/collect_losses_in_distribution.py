...
# def compute_loss_single(diffusion, normalizers, obs_raw, action_raw, device,
#                         batch_multiplier, num_per_batch):
#     """
#     Compute avg diffusion loss for a single (obs, action) pair.
#     Returns a scalar float.
#     """
#     from diffusha.diffusion.utils import extract

#     obs_n    = normalize_obs(obs_raw, normalizers).to(device)        # [obs_dim]
#     action_n = normalize_action(action_raw, normalizers).to(device)  # [action_dim]

#     # shape expected by repeat: [1, 1, dim]
#     nobs    = obs_n.unsqueeze(0).unsqueeze(0)     # [1, 1, obs_dim]
#     naction = action_n.unsqueeze(0).unsqueeze(0)  # [1, 1, action_dim]

#     N = diffusion.num_diffusion_steps * batch_multiplier

#     nobs_repeat    = nobs.repeat(N, 1, 1)     # [N, 1, obs_dim]
#     naction_repeat = naction.repeat(N, 1, 1)  # [N, 1, action_dim]
...
#         # joint state: [N, 1, obs_dim + action_dim]
#         x_0   = torch.cat([nobs_repeat, naction_repeat], dim=-1)
#         noise = torch.empty_like(x_0).normal_(0, 1)

#         # manually apply noise with pre-generated noise
#         a   = extract(diffusion.alphas_bar_sqrt,           timesteps, x_0)
#         am1 = extract(diffusion.one_minus_alphas_bar_sqrt, timesteps, x_0)
#         noisy_x = x_0 * a + noise * am1

#         output = diffusion.model(noisy_x, timesteps)
#         err    = noise - output
...
#     """
#     Compute avg diffusion loss for a single (obs, action) pair.
#     No normalization — model was trained on raw values.
#     Returns a scalar float.
#     """
#     from diffusha.diffusion.utils import extract
