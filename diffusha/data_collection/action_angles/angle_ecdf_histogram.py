...
def main():
    ...
    ax.hist(target_arr, bins=60, alpha=0.5, weights=np.ones(len(target_arr))/len(target_arr), label="target (ID)")
    ax.hist(target_flipped_arr, bins=60, alpha=0.5, weights=np.ones(len(target_flipped_arr))/len(target_flipped_arr), label="flipped (OOD)")
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig("theta_histogram_prop.png", dpi=150)
    print("saved theta_histogram_prop.png")
