...
from diffusha.data_collection.env.utils import xarm_sim_robot
...
BLOCK_URDF_PATH = 'diffusha/data_collection/env/assets/block.urdf'
PLANE_URDF_PATH = ('third_party/bullet/examples/pybullet/gym/pybullet_data/'
                   'plane.urdf')
WORKSPACE_URDF_PATH = 'diffusha/data_collection/env/assets/workspace.urdf'
ZONE_URDF_PATH = 'diffusha/data_collection/env/assets/zone.urdf'
INSERT_URDF_PATH = 'diffusha/data_collection/env/assets/insert.urdf'
...
WORKSPACE_URDF_PATH_REAL = 'diffusha/data_collection/env/assets/workspace_real.urdf'
...
CAMERA_INTRINSICS_REAL = (
    ...
    0,
    IMAGE_WIDTH_REAL / 2.,  # cx
    0,
    0.803 * IMAGE_WIDTH_REAL,  # fy
    IMAGE_HEIGHT_REAL / 2.,  # cy
    0,
    0,
    1)
...
class BlockPush(gym.Env):
  ...
  def get_metrics(self, num_episodes):
    metrics = [
        block_pushing_metrics.AverageFinalGoalDistance(
            self, buffer_size=num_episodes),
        block_pushing_metrics.AverageSuccessMetric(
            self, buffer_size=num_episodes)
    ]
  ...
  def __init__(self,
    ...
    self._step_frequency = 1 / self._pybullet_client.getPhysicsEngineParameters(
    )['fixedTimeStep']
  ...
  def _setup_pybullet_scene(self):
    ...
    self._block_ids = [utils_pybullet.load_urdf(
  ...
  def reset(self, reset_poses = True):
    ...
    if reset_poses:
      ...
      self._pybullet_client.resetBasePositionAndOrientation(
          self._block_ids[0], block_translation.tolist(),
      ...
      self._pybullet_client.resetBasePositionAndOrientation(
          self._target_id, target_translation.tolist(),
    ...
    self._target_pose = Pose3d(rotation=target_rotation,
  ...
  def _compute_goal_distance(self, state):
    ...
    if self._task != BlockTaskVariant.REACH:
      goal_distance = np.linalg.norm(state['block_translation'] -
                                     goal_translation[0:2])
    else:
      goal_distance = np.linalg.norm(state['effector_translation'] -
                                     goal_translation[0:2])
  ...
  def _compute_reach_target(self, state):
    ...
    self.reach_target_translation = (xy_block +
  ...
  def _step_robot_and_sim(self, action):
    ...
    target_effector_pose = Pose3d(rotation=EFFECTOR_DOWN_ROTATION,
    ...
    # TODO(oars, peteflorence): Is there a way to directly step n times?
  ...
  def get_pybullet_state(self):
    ...
    state['robots'] = [XarmState.get_bullet_state(
    ...
    if self.robot.end_effector:
      state['robot_end_effectors'].append(ObjState.get_bullet_state(
          self._pybullet_client, self.robot.end_effector))

    state['targets'] = []
    if self._target_id:
      state['targets'].append(ObjState.get_bullet_state(
          self._pybullet_client, self._target_id))
  ...
  def set_pybullet_state(
    ...
    _set_state_safe(
        None if not targets else targets[0],
        self._target_id)
...
class BlockPushNormalized(gym.Env):
  ...
  def __init__(self,
    ...
    self._env = BlockPush(control_frequency, env_task, image_size,
                          shared_memory, seed)
  ...
  def step(self, action):
    ...
    # TODO(tompson): peteflorence@ suggestion: try clipping with L2 magnitude.
  ...
  def calc_unnormalized_state(
    ...
    effector_target_to_block_translation = cls._unnormalize(
        ...
        EFFECTOR_TARGET_TO_BLOCK_TRANSLATION_MIN,
        EFFECTOR_TARGET_TO_BLOCK_TRANSLATION_MAX)
    block_translation = (effector_target_to_block_translation +
                         effector_target_translation)
    ori_cos_sin = cls._unnormalize(norm_state['block_orientation_cos_sin'],
                                   BLOCK_ORIENTATION_COS_SIN_MIN,
                                   BLOCK_ORIENTATION_COS_SIN_MAX)
    block_orientation = np.array([math.atan2(ori_cos_sin[1], ori_cos_sin[0])],
                                 np.float32)
    ...
    effector_target_to_target_translation = cls._unnormalize(
        ...
        EFFECTOR_TARGET_TO_TARGET_TRANSLATION_MIN,
        EFFECTOR_TARGET_TO_TARGET_TRANSLATION_MAX)
    target_translation = (effector_target_to_target_translation +
                          effector_target_translation)
    ori_cos_sin = cls._unnormalize(norm_state['target_orientation_cos_sin'],
                                   TARGET_ORIENTATION_COS_SIN_MIN,
                                   TARGET_ORIENTATION_COS_SIN_MAX)
    target_orientation = np.array([math.atan2(ori_cos_sin[1], ori_cos_sin[0])],
                                  np.float32)
...
# debugging.
# TODO(ayzaan, peteflorence): Consider creating environment class directly
# rather than using gym's registration feature so that the number of different
# environment names doesn't blow up with added environment features.
...
registration.register(
    id='BlockPush-v0', entry_point=BlockPush, max_episode_steps=100)
registration.register(
...
registration.register(
    ...
    max_episode_steps=100,
    kwargs=dict(
        task=BlockTaskVariant.REACH, image_size=(IMAGE_HEIGHT, IMAGE_WIDTH)))
registration.register(
...
registration.register(
    ...
    entry_point=BlockPushNormalized,
    kwargs=dict(
        task=BlockTaskVariant.REACH_NORMALIZED,
        image_size=(IMAGE_HEIGHT, IMAGE_WIDTH)),
    max_episode_steps=50)
